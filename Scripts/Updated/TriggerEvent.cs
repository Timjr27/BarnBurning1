...
public class TriggerEvent : MonoBehaviour
{
    ...
    public AudioSource zitat;
    private AudioSource currentZitat;
    public List<AudioSource> zitate;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        zitat = this.GetComponent<AudioSource>();
        Debug.Log("SOUNDDDDDD");
        if (zitat!=null)
        {
            Debug.Log("Step2");
            if (!zitate.Contains(zitat))
            {
                Debug.Log("Step3");

                zitate.Add(zitat);
                Invoke("PlayWithDelay", 1.5f);
                //zitat.Play();
